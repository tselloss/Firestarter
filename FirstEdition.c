...
void *calculateBurnFirstEdition(void *arg){
    int **forest;
    int i_trial;
    int i_prob;
    double local_percent_burned;
    int thread_id = (int) arg,
            step = n_probs/NUMBER_THREADS,
            start = step * thread_id,
            end = start+step;
    if (thread_id == NUMBER_THREADS-1) end++;
    printf("Hello from thread %d, start %d and end %d with step %d\n",thread_id, start, end, step);
    // setup problem
    seed_by_time(0);
    ...
    for (i_prob = start ; i_prob < end; i_prob++) {
...
//        percent_burned[i_prob]=0.0;
        ...
        percent_burned[i_prob]=local_percent_burned/n_trials;
...
int main(int argc, char ** argv) {
    ...
    for (i = 0; i < NUMBER_THREADS; ++i) {
        pthread_create(&threads[i], NULL, calculateBurnFirstEdition, (void *) i);
    }
    for (i = 0; i < NUMBER_THREADS; ++i) {
